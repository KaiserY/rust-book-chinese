...
要修正这个问题的话，我们必须确保第四步永远也不在第三步之后发生。Rust所有权系统通过一个叫*生命周期*（*lifetime*）的概念来做到这一点，它定义了一个引用有效的作用域。
...
## 'static
叫做`static`的作用域是特殊的。它代表某样东西具有横跨整个程序的生命周期。大部分Rust程序员当他们处理字符串时第一次遇到`'static`：
