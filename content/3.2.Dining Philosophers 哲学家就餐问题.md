...
在线程中，所有我们做的就是在`p`上调用`eat()`。另外注意到`thread::spawn`调用最后木有分号，这使它是一个表达式。这个区别是重要的，以便生成正确的返回值。更多细节，请看[表达式VS语句](https://doc.rust-lang.org/stable/book/functions.html#expressions-vs.-statements)。
