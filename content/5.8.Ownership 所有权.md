...
## `Copy`类型
我们已经建立起了当所有权被转移给另一个绑定时，你不能使用原始绑定的观念后。然而，这里有一个[trait](http://doc.rust-lang.org/stable/book/traits.html)会改变这个行为，它叫做`Copy`。我们还没有讨论到trait，不过目前，你可以理解为一个为特定类型增加额外行为的标记。例如：
