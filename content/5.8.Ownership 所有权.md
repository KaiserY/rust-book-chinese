...
## `Copy`类型
我们已经知道了当所有权被转移给另一个绑定以后，你不能再使用原始绑定。然而，这里有一个[特性](http://doc.rust-lang.org/stable/book/traits.html)会改变这个行为，它叫做`Copy`。我们还没有讨论到特性，不过目前，你可以理解为一个为特定类型增加额外行为的标记。例如：
