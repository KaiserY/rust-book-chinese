...
> [the-stack-and-the-heap.md](https://github.com/rust-lang/book/blob/master/first-edition/src/the-stack-and-the-heap.md)
> <br>
...
在 Rust 中，你可以使用[`Box<T>`类型](http://doc.rust-lang.org/std/boxed/index.html)在堆上分配内存。这是一个例子：
...
大部分语言有一个默认堆分配的垃圾回收器。这意味着每个值都是装箱的。有很多原因为什么要这么做，不过这超出了这个教程的范畴。这也有一些优化会使得这些规则不是 100% 的时间都成立。垃圾回收器用处理堆来代替依赖栈和`Drop`来清理内存的方式。
