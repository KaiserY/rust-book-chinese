...
关于trait的最后一点：带有trait限制的泛型函数是*单态*（*monomorphization*）（mono：单一，morph：形式）的，所以它是*静态分发*（*statically dispatched*）的。这是神马意思？查看[trait对象](http://doc.rust-lang.org/nightly/book/trait-objects.html)来了解更多细节。
...
这突显出了`where`从句的额外的trait：它允许限制的左侧可以是任意类型（在这里是`i32`），而不仅仅是一个类型参数（比如`T`）。
...
`Foo`trait需要实现`bar()`，不过并不需要实现`baz()`。它会使用默认的行为。如果你想的话也能覆盖默认行为：
