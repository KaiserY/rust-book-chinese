...
我们用`rand::thread_rng()`函数来获取一个随机数生成器的拷贝，它位于我们特定的执行[线程](https://doc.rust-lang.org/stable/book/concurrency.html)的本地。因为我们`use rand::Rng`了，我们有一个`gen_range()`方法可用。这个函数获取两个参数，并产生一个位于其间的数字。它包含下限，不过不包含上限，所以我们需要`1`和`101`来生成一个`1`和`100`之间的数。

第二行仅仅打印出了秘密数字。这在我们开发我们的程序时很有用，所以我们可以简单地测试出来。不过在最终版本中我们会删除它。在开始就打印出结果就没什么可玩的了！
...
[Ordering](https://doc.rust-lang.org/stable/std/cmp/enum.Ordering.html)枚举有3个可能的变量：`Less`，`Equal`和`Greater`。`match`语句获取类型的值，并让你为每个可能的值创建一个“分支”。因为我们有3种类型的`Ordering`，我们有3个分支：
