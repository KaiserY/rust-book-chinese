...
这一行以一个分号结尾（`;`）。Rust是一门[**面向表达式**](Glossary 词汇表#面向表达式语言（expression-oriented-language）)的语言，也就是说大部分语句都是表达式。`;` 表示一个表达式的结束，另一个新表达式的开始。大部分 Rust 代码行以`;`结尾。
...
让我们将 Hello World 程序迁移至 Cargo。为了 Cargo 化一个项目，你需要做三件事：
...
Cargo 期望你的源文件位于 src 目录，所以先做这个。这样将项目顶级目录（在这里，是 hello_world）留给 README，license 信息和其他跟代码无关的文件。这样，Cargo 帮助你保持项目干净整洁。一切井井有条。
...
当你的项目最终准备好发布了，你可以使用`cargo build --release`来优化编译你的项目。这些优化可以让你的 Rust 代码运行的更快，不过启用他们会让你的程序花更长的时间编译。这也是为何这是两种不同的配置，一个为了开发，另一个构建提供给用户的最终程序。
