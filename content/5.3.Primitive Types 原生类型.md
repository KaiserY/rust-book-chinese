...
## `str`
Rust的`str`类型是最原始的字符串类型。作为一个[不定长类型](http://doc.rust-lang.org/nightly/book/unsized-types.html)，它本身并不是非常有用，不过当它用在引用后是就有用了，例如[&str](http://doc.rust-lang.org/nightly/book/strings.html)。如你所见，我们到时候在讲。
