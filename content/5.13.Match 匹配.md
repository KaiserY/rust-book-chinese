...
`match`使用一个表达式然后基于它的值分支。每个分支都是`val => expression`这种形式。当匹配到一个分支，它的表达式将被执行。`match`属于“模式匹配”的范畴，`match`是它的一个实现。这里有[一个整个关于模式的部分](https://doc.rust-lang.org/stable/book/patterns.html)讲到了素有可能的模式。
